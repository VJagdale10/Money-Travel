...
class _HomeState extends State<HomeWidget> {
  ...
  loadValues() {
    // System Values
    _readSP("currency").then((val) {
      setState(() {
        stringData["currency"] = val == null ? "USD" : val;
      });
    });
    _readSP("dailyLimit").then((val) {
      setState(() {
        numData["dailyLimit"] = val == null ? 0 : val;
      });
    });
    _readSP("totalSaved").then((val) {
      setState(() {
        numData["totalSaved"] = val == null ? 0 : val;
      });
    });
    _readSP("todayDate").then((todayDate) {
      setState(() {
        dynamic date = todayDate == null
            ? DateTime.now().millisecondsSinceEpoch
            : todayDate;

        print("todayDate" + todayDate.toString());
        numData["todayDate"] = date;
        // Query Today Spendings from DB
        _queryDBDay(date).then((entries) {
    ...
    // UI Parameters
    _readSP("showSave").then((val) {
      setState(() {
        numData["showSave"] = val;
      });
    });
    _readSP("historyMode").then((val) {
      setState(() {
        numData["historyMode"] = val == null ? 0 : val;
      });
    });
    _readSP("version").then((val) {
      setState(() {
        numData["version"] = val;
      });
    });
  ...
  void checkNewDay() {
    ...
    if (numData["todayDate"] != today) {
      DateTime prevDate =
          DateTime.fromMillisecondsSinceEpoch(numData["todayDate"]);
      ...
      for (DateTime dt in calculateDaysInterval(
          prevDate, DateTime.fromMillisecondsSinceEpoch(today))) {
